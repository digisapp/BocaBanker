    ...
    <div className="animate-fade-in max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-[#F8FAFC]">{study.studyName}</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent className="bg-[#1A2B45] border-[rgba(201,168,76,0.15)]">
              <AlertDialogHeader>
                <AlertDialogTitle className="text-[#F8FAFC]">Delete Study</AlertDialogTitle>
      ...
      {!hasResults && (
        <div className="glass-card p-6">
          <h3 className="text-lg font-semibold text-[#F8FAFC] mb-4">Study Details</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
            <div>
              <span className="text-xs text-[#64748B]">Property</span>
              <p className="text-sm font-medium text-[#F8FAFC] mt-1">
                {study.propertyName || '-'}
              </p>
            </div>
            <div>
              <span className="text-xs text-[#64748B]">Property Type</span>
              <p className="text-sm font-medium text-[#F8FAFC] mt-1 capitalize">
                {study.propertyType?.replace('_', ' ') || '-'}
              </p>
            </div>
            <div>
              <span className="text-xs text-[#64748B]">Purchase Price</span>
              <p className="text-sm font-medium text-[#F8FAFC] mt-1">
                {formatCurrency(study.purchasePrice)}
              </p>
            </div>
            <div>
              <span className="text-xs text-[#64748B]">Study Year</span>
              <p className="text-sm font-medium text-[#F8FAFC] mt-1">{study.studyYear}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6 mb-6">
            <div>
              ...
              <p className="text-sm font-medium text-[#F8FAFC] mt-1">{study.taxRate}%</p>
            </div>
            <div>
              <span className="text-xs text-[#64748B]">Discount Rate</span>
              <p className="text-sm font-medium text-[#F8FAFC] mt-1">{study.discountRate || '5'}%</p>
            </div>
            <div>
              <span className="text-xs text-[#64748B]">Bonus Depreciation</span>
              <p className="text-sm font-medium text-[#F8FAFC] mt-1">{study.bonusDepreciationRate || '100'}%</p>
            </div>
          </div>
          ...
          {assets.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-[#C9A84C] mb-3">Study Assets</h4>
              <div className="space-y-2">
                {assets.map((asset) => (
                  <div
                    key={asset.id}
                    className="flex items-center justify-between p-3 rounded-lg bg-[rgba(15,27,45,0.5)] border border-[rgba(201,168,76,0.08)]"
                  >
                    <div>
                      <p className="text-sm text-[#F8FAFC]">{asset.assetName}</p>
          ...
          {assets.length === 0 && (
            <div className="text-center py-8 border border-dashed border-[rgba(201,168,76,0.15)] rounded-lg">
              <FileBarChart className="h-8 w-8 text-[#64748B] mx-auto mb-2" />
