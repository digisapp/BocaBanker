...
export default function StudyForm({
  ...
  // Auto-populate client and assets when property is selected
  const populateFromProperty = useCallback((prop: PropertyOption) => {
    // Auto-set client if property has one
    if (prop.clientId) {
      setClientId(prop.clientId)
    }

    // Auto-populate study name
    if (!studyName) {
      setStudyName(`Cost Seg Study - ${prop.address}`)
    }

    // Generate default asset allocation based on property type
    const purchasePrice = typeof prop.purchasePrice === 'string'
      ? parseFloat(prop.purchasePrice)
      : prop.purchasePrice

    try {
      // Normalize property type for lookup
      ...
      // Fallback to commercial allocation
    ...
    <div className="space-y-6">
                  ...
                  <div
                    className={`flex h-10 w-10 items-center justify-center rounded-full border-2 transition-all ${
                      isActive
                        ...
                        : isComplete
                        ? 'border-[#C9A84C] bg-[#C9A84C] text-[#0F1B2D]'
                        : 'border-[rgba(201,168,76,0.15)] text-[#64748B]'
                    }`}
                  >
                    {isComplete ? (
                ...
                {index < STEPS.length - 1 && (
                  <div
                    className={`hidden sm:block w-16 md:w-24 h-[2px] mx-2 mt-[-16px] ${
                      isComplete ? 'bg-[#C9A84C]' : 'bg-[rgba(201,168,76,0.15)]'
                    }`}
                  />
                )}
              </div>
      ...
      {currentStep === 0 && (
        <div className="glass-card p-6 space-y-6">
          <h3 className="text-lg font-semibold text-[#F8FAFC]">Select Property</h3>

          <div className="space-y-4">
            <div>
              <Label className="text-[#94A3B8]">Study Name</Label>
              <Input
                value={studyName}
                onChange={(e) => setStudyName(e.target.value)}
                placeholder="Cost Seg Study - 123 Main St"
                className="mt-1.5 bg-[#0F1B2D] border-[rgba(201,168,76,0.15)] text-[#F8FAFC] placeholder:text-[#475569]"
              />
              {errors.study_name && (
                <p className="text-xs text-red-400 mt-1">{errors.study_name}</p>
              )}
            </div>

            <div>
              <Label className="text-[#94A3B8]">Property</Label>
              <Select value={propertyId} onValueChange={setPropertyId}>
                <SelectTrigger className="mt-1.5 w-full bg-[#0F1B2D] border-[rgba(201,168,76,0.15)] text-[#F8FAFC]">
                  <SelectValue placeholder="Select a property" />
                </SelectTrigger>
                <SelectContent className="bg-[#1A2B45] border-[rgba(201,168,76,0.15)]">
                  {properties.map((prop) => (
                    <SelectItem key={prop.id} value={prop.id} className="text-[#F8FAFC] focus:bg-[rgba(201,168,76,0.1)] focus:text-[#C9A84C]">
              ...
              {errors.property_id && (
                <p className="text-xs text-red-400 mt-1">{errors.property_id}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.client_id && (
                <p className="text-xs text-red-400 mt-1">{errors.client_id}</p>
              )}
            </div>
          </div>
          ...
          {selectedProperty && (
            <div className="mt-6 p-4 rounded-lg bg-[rgba(15,27,45,0.5)] border border-[rgba(201,168,76,0.12)]">
              <h4 className="text-sm font-medium text-[#C9A84C] mb-3">Property Details</h4>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <span className="text-[#64748B]">Address</span>
                  <p className="text-[#F8FAFC]">{selectedProperty.address}</p>
                </div>
                <div>
                  <span className="text-[#64748B]">Type</span>
                  <p className="text-[#F8FAFC] capitalize">{selectedProperty.propertyType.replace('_', ' ')}</p>
                </div>
                <div>
                  <span className="text-[#64748B]">Purchase Price</span>
                ...
                <div>
                  <span className="text-[#64748B]">Building Value</span>
                  <p className="text-[#F8FAFC]">
      ...
      {currentStep === 1 && (
        <div className="glass-card p-6 space-y-6">
          <h3 className="text-lg font-semibold text-[#F8FAFC]">Tax Parameters</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input
                ...
                max={100}
                step={0.1}
                className="mt-1.5 bg-[#0F1B2D] border-[rgba(201,168,76,0.15)] text-[#F8FAFC]"
              />
              {errors.tax_rate && <p className="text-xs text-red-400 mt-1">{errors.tax_rate}</p>}
              <p className="text-xs text-[#475569] mt-1">Federal marginal tax rate</p>
            </div>

            <div>
              <Label className="text-[#94A3B8]">Discount Rate (%)</Label>
              <Input
                type="number"
                value={discountRate}
              ...
              <p className="text-xs text-[#475569] mt-1">For NPV calculation</p>
            </div>

            <div>
              <Label className="text-[#94A3B8]">Bonus Depreciation Rate (%)</Label>
              <Input
                type="number"
                value={bonusDepreciationRate}
                onChange={(e) => setBonusDepreciationRate(Number(e.target.value))}
                min={0}
                max={100}
                step={1}
                className="mt-1.5 bg-[#0F1B2D] border-[rgba(201,168,76,0.15)] text-[#F8FAFC]"
              ...
              <p className="text-xs text-[#475569] mt-1">Section 168(k) rate for eligible assets</p>
            </div>

            <div>
              <Label className="text-[#94A3B8]">Study Year</Label>
              <Input
                type="number"
                value={studyYear}
      ...
      {currentStep === 2 && (
        <div className="glass-card p-6 space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-[#F8FAFC]">Asset Breakdown</h3>
            <div className="text-sm text-[#64748B]">
              Total: <span className="text-[#C9A84C] font-medium">{formatCurrency(totalAssetValue)}</span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {/* Header */}
            <div className="grid grid-cols-12 gap-3 px-3 text-xs text-[#64748B] uppercase tracking-wide">
            ...
            {assets.map((asset, index) => {
              const pct = totalAssetValue > 0
                ? ((asset.amount / totalAssetValue) * 100).toFixed(1)
                : '0.0'

              return (
                <div
                  key={index}
                  className="grid grid-cols-12 gap-3 items-center p-3 rounded-lg bg-[rgba(15,27,45,0.5)] border border-[rgba(201,168,76,0.08)]"
                >
                  <div className="col-span-4">
                    ...
                    >
                      <SelectTrigger className="bg-transparent border-[rgba(201,168,76,0.1)] text-[#F8FAFC] text-sm h-8">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="bg-[#1A2B45] border-[rgba(201,168,76,0.15)]">
                        {ASSET_CLASSES.map((ac) => (
                          <SelectItem key={ac.category} value={ac.category} className="text-[#F8FAFC] focus:bg-[rgba(201,168,76,0.1)] focus:text-[#C9A84C] text-sm">
                            {ac.description}
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  <div className="col-span-3">
                    <div className="relative">
                      <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[#C9A84C] text-xs">$</span>
                      <Input
                        type="number"
                        value={asset.amount || ''}
                        onChange={(e) => updateAssetAmount(index, Number(e.target.value))}
                        className="bg-transparent border-[rgba(201,168,76,0.1)] text-[#F8FAFC] text-sm h-8 pl-5"
                      />
                    </div>
                  </div>
      ...
      {currentStep === 3 && (
        <div className="space-y-6">
          <div className="glass-card p-6">
            <h3 className="text-lg font-semibold text-[#F8FAFC] mb-4">Review Study</h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <span className="text-xs text-[#64748B] uppercase tracking-wide">Study Name</span>
                <p className="text-sm font-medium text-[#F8FAFC] mt-1">{studyName}</p>
              </div>
              <div>
                <span className="text-xs text-[#64748B] uppercase tracking-wide">Property</span>
              ...
              <div>
                <span className="text-xs text-[#64748B] uppercase tracking-wide">Study Year</span>
                <p className="text-sm font-medium text-[#F8FAFC] mt-1">{studyYear}</p>
              </div>
            </div>
          </div>

          <div className="glass-card p-6">
            <h4 className="text-sm font-semibold text-[#C9A84C] mb-3">Tax Parameters</h4>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <span className="text-xs text-[#64748B]">Tax Rate</span>
                <p className="text-sm font-medium text-[#F8FAFC]">{taxRate}%</p>
              </div>
              <div>
                <span className="text-xs text-[#64748B]">Discount Rate</span>
                <p className="text-sm font-medium text-[#F8FAFC]">{discountRate}%</p>
              </div>
              <div>
                <span className="text-xs text-[#64748B]">Bonus Depreciation</span>
                <p className="text-sm font-medium text-[#F8FAFC]">{bonusDepreciationRate}%</p>
              </div>
            </div>
          </div>

          <div className="glass-card p-6">
            <h4 className="text-sm font-semibold text-[#C9A84C] mb-3">Asset Allocation</h4>
            <div className="space-y-2">
              {assets.map((asset, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-[rgba(201,168,76,0.08)] last:border-0">
                  <span className="text-sm text-[#94A3B8]">{asset.description}</span>
                  <span className="text-sm font-medium text-[#F8FAFC]">{formatCurrency(asset.amount)}</span>
                </div>
              ))}
              <div className="flex items-center justify-between pt-3 border-t border-[rgba(201,168,76,0.15)]">
                <span className="text-sm font-semibold text-[#C9A84C]">Total</span>
                <span className="text-sm font-bold text-[#C9A84C]">{formatCurrency(totalAssetValue)}</span>
              </div>
            </div>
          </div>
        </div>
